   ...
   return (
      <div className={props.active ? style.modal : style.hidden}>
         ...
         {product.map((el)=>(
            ...
            <img src={el.thumbnail} alt="Product Title"  className={style.modal_image}/>
            <p><strong>Price: {el.price} $</strong></p>
            <p><strong>Brand: {el.brand}</strong></p>
            <p><strong>Model: {el.title}</strong></p>
            ...
            <Link to={"/"} onClick={()=>addProductToCart(el.id)}>Continue</Link>
            <button
